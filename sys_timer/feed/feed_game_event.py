...
async def process_feed_game_event(bot: commands.Bot) -> int:
    ...
    log(f"[FEED - EVENT]: [CHANNELS: x{channel_count}] [ARTICLES: x{article_count}]", "feed", show=False)

    if channel_count == 0:
        log(f"[FEED - EVENT]: [ERROR: NO CHANNELS] ", "feed", level="WARN", show=False)
    ...
    if not new_hashes:
        ...
        log(f"[FEED - EVENT]: [ERROR: NO ARTICLES] ", "feed", level="WARN", show=False)
        log(f"[FEED - EVENT]: Thread closed.", "feed", show=False)
        log(f"", "feed", show=False)
        return 0
    ...
    log(f"[FEED - EVENT]: [ARTICLES: x{len(new_articles)}]", "feed", show=False)
    ...
    log(f"[FEED - EVENT]: [SENT x{new_count}]", "feed", show=False)
    log(f"[FEED - EVENT]: Thread closed.", "feed", show=False)
    log(f" ", "feed", show=False)
    return new_count