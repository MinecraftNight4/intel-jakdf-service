...
def build_gacha_info_text(article: dict) -> Optional[str]:
    """
    Intenta interpretar si la noticia indexada es
    valida con alguna plantilla de noticia.
    """
    # ARTICLE - VALUE CHECK
    article_item = article.get("article_item") or []
    article_unix = article.get("article_unix") or []
    article_text = "\n".join(str(i) for i in article_item).lower()
    try:
        rule_unix = sorted(int(unix) for unix in article_unix if unix is not None)
    ...
    # ARTICLE TYPE FILTER:
    # GACHA PULL | EXCHANGE + AVAILABILITY + 3 UNIX
    if rule_pull and rule_shop and len(rule_unix) == 3:
        time_1, time_2, time_3 = rule_unix
        return (
            f"- [REPLACE_RELEASE] <t:{time_1}:f> (<t:{time_1}:R>)\n"
            f"- [REPLACE_GACHA_LEAVE] <t:{time_2}:f> (<t:{time_2}:R>)\n"
            f"- [REPLACE_STORE_LEAVE] <t:{time_3}:f> (<t:{time_3}:R>)"
        )

    # EVENT PULL | !EXCHANGE + AVAILABILITY + 3 UNIX
    if rule_pull and not rule_shop and len(rule_unix) == 3:
        time_1, time_2, time_3 = rule_unix
        return (
            f"- [REPLACE_RELEASE] <t:{time_1}:f> (<t:{time_1}:R>)\n"
            f"- [REPLACE_EVENT_LEAVE] <t:{time_2}:f> (<t:{time_2}:R>)\n"
            f"- [REPLACE_GACHA_LEAVE] <t:{time_3}:f> (<t:{time_3}:R>)"
        )
    
    # SPECIAL EVENT | !EXCHANGE + AVAILABILITY + 2 UNIX
    if rule_pull and not rule_shop and len(rule_unix) == 2:
        time_1, time_2 = rule_unix
        return (
            f"- [REPLACE_RELEASE] <t:{time_1}:f> (<t:{time_1}:R>)\n"
            f"- [REPLACE_GACHA_LEAVE] <t:{time_2}:f> (<t:{time_2}:R>)"
        )
    
    # NOT COMPATIBLE WITH CONTENT
    return None
...
class FeedGachaPageView(ui.LayoutView):
    def __init__(
        ...
        article_rgbs = item_rgb if item_rgb is not None else int(article.get("article_rgbs", "ffffff"), 16)
        ...
        # EMBED - BODY
        container.add_item(ui.TextDisplay(item_txt))
        container.add_item(ui.Separator())
        ...
        row.add_item(ui.Button(
            ...
            custom_id=f"private_gamenews_{article_uuid}_1"
        ))
...
async def process_feed_game_gacha(bot: commands.Bot) -> int:
    ...
    process_news = [
        ...
        if (read.get("article_type") or "").lower() == "gacha"
    ...
    for guild_uuid, guild_data in storage_feed.items():
        ...
        for item in process_post:
            message_text = build_gacha_info_text(item)
            if message_text is None:
                log(f"  - [(!) FAILURE] #ERROR_FLAG_0001 | #{item.get('article_hash')}] - {item.get('article_name')} | DUMP: The article doesn't fit on the templates.", "feed", level="CRIT", show=False)
                continue
            ...
            message_embd = FeedGachaPageView(item, item_txt=message_text, item_ico=message_icon, item_rgb=message_rgbs)
            try:
                for file_item in message_file:
                    file_item.fp.seek(0)
                message_sent = await asyncio.wait_for(feed_data_hold.send(view=message_embd, files=message_file if message_file else None), timeout=10.0)
                ...
                log(f"  - [(!) FAILURE] #ERROR_FLAG_0003 | #{item.get('article_hash')}] - {item.get('article_name')} | DUMP: {e}", "feed", level="CRIT", show=False)
                continue
            debug_post_sent += 1
        ...
        if feed_ping_text and str(feed_ping_text).strip():
            ...
            try:
                ...
                log(f"    ⤷ [(!) FAILURE] #ERROR_FLAG_0004 | DUMP: {e} ", "feed", level="CRIT", show=False)
                continue
